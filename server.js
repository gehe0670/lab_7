...
app.get('/team_stats', function(req, res){
  ...
  var query3 = 'SELECT COUNT(*) AS loses FROM football_games WHERE home_score < visitor_score';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
...
// info render for player-info
app.get('/player_info/post', function(req, res){
  var player_choice = req.query.player_choice
  var query1 = 'SELECT id, name FROM football_players;';
  var query2 = 'SELECT year, major, passing_yards, rushing_yards, receiving_yards, img_src FROM football_players WHERE id = ' + player_choice + ';';
  var query3 = 'SELECT COUNT(*) AS games_played FROM football_games WHERE ' + player_choice + ' = ANY(players);';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
